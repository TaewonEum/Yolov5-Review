...
Convolutional 레이어를 통한 특징 추출: Backbone에서 추출한 특징 맵을 Head의 다양한 Convolutional 레이어에 입력으로 넣어 다양한 크기와 추상화 수준의 특징을 얻습니다.

Detection Layer를 통한 예측 생성: 각 Convolutional 레이어 뒤에는 Detection Layer가 따라오며, 이 레이어에서는 앵커 박스의 개수와 클래스 수에 따라 예측된 바운딩 박스와 클래스 확률을 생성합니다.

실제 객체와 예측 객체의 IOU 및 Loss 계산: 예측된 바운딩 박스와 실제 객체 간의 IoU(Intersection over Union)를 계산하고, 이를 기반으로 위치 오차와 크기 오차를 계산하여 Loss를 구합니다. 이 Loss는 객체의 위치와 크기, 그리고 클래스 예측의 정확도를 평가하는 데 사용됩니다.

Loss 역전파 및 가중치 업데이트: 계산된 Loss를 사용하여 네트워크의 파라미터를 업데이트하기 위해 역전파를 수행합니다. 이를 통해 네트워크는 예측된 바운딩 박스와 실제 객체 간의 오차를 줄이는 방향으로 학습됩니다. 가중치가 업데이트되어 예측의 정확도와 위치 정확도를 향상시킵니다.

예측 후처리: 학습된 모델을 이용하여 입력 이미지에서 객체를 예측합니다. 이때 Non-Maximum Suppression(NMS) 등의 기법을 사용하여 겹치는 예측 박스를 제거하고, 가장 정확한 박스를 선택합니다.
